...
\begin{center}
  ...
  \vspace{0.55em}
  {\normalsize Brandon Kirkland}\par
  \vspace{0.2em}
...
equations have a real solution.  A Banach contraction calculation with final
rational bounds establishes its existence under the stated arithmetic assumptions.  The tested
...
A separate a posteriori Banach contraction calculation uses
...
the project roadmap included with the repository.

\section{Computational artifacts and reproduction}

The tested command-line pipeline performs the catalog construction,
representation calculations, minimal-valise search, and contraction-bound
...
\artifactpath{results/sr_joint_root_audit.json} & Banach contraction bounds for
the projected linkage equations. \\
