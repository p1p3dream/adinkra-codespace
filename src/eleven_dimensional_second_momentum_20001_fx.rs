...
pub(crate) enum SecondMomentum20001GpuColumnEvent {
    WordLoweringStart {
...
struct PreparedGpuColumn {
    ...
    preflight: SecondMomentum20001GpuColumnPreflight,
}
...
fn pbw_plan_sha256(words: &[Vec<u8>]) -> String {
    ...
    hash.update(b"adynkra-11d-second-momentum-20001-pbw-plan-v1\0");
...
fn prepare_gpu_column(local_ordinal: usize) -> io::Result<PreparedGpuColumn> {
    ...
    if words.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "empty exact (20001) reciprocal PBW plan",
        ));
    }
    ...
    let preflight = SecondMomentum20001GpuColumnPreflight {
...
pub(crate) fn gpu_column_preflight(
    local_ordinal: usize,
) -> io::Result<SecondMomentum20001GpuColumnPreflight> {
    ...
    F: FnMut(crate::eleven_dimensional_second_momentum_gpu::RecoupledSourceTerm) -> io::Result<()>,
{
    ...
    visit_prepared_gpu_column_events_from(prepared, 0, |event| match event {
        SecondMomentum20001GpuColumnEvent::Term { term, .. } => visit(term),
        SecondMomentum20001GpuColumnEvent::WordLoweringStart { .. }
        | SecondMomentum20001GpuColumnEvent::WordStart { .. }
        | SecondMomentum20001GpuColumnEvent::WordEnd { .. } => Ok(()),
    })
}
    ...
    F: FnMut(SecondMomentum20001GpuColumnEvent) -> io::Result<()>,
{
    ...
    if prepared.preflight != *expected_preflight {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "(20001) GPU column preflight identity changed before streaming",
        ));
    }
    ...
    F: FnMut(SecondMomentum20001GpuColumnEvent) -> io::Result<()>,
{
    ...
    if prepared.preflight != *expected_preflight {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "(20001) GPU column preflight identity changed before opaque streaming",
        ));
    }
            ...
            |event| match event {
                crate::eleven_dimensional_level16_couplings::CoupledWordStateEvent::
                    WordLoweringStart { ordinal, pbw_word } => {
                        visit(SecondMomentum20001GpuColumnEvent::WordLoweringStart {
                ...
                crate::eleven_dimensional_level16_couplings::CoupledWordStateEvent::WordStart {
                    ...
                    visit(SecondMomentum20001GpuColumnEvent::WordStart {
                        requested_word_ordinal: ordinal,
                        pbw_word_simple_roots: pbw_word.to_vec(),
                    })?;
                    Ok(0)
                ...
                crate::eleven_dimensional_level16_couplings::CoupledWordStateEvent::State {
                    ...
                    let reported = download_terms(state, &mut |key, descendant_coefficient| {
                            ...
                            || previous_key.is_some_and(|previous| previous >= key)
                        {
                            return Err(io::Error::new(
                                io::ErrorKind::InvalidData,
                                "downloaded (20001) terminal state is not canonical",
                            ));
                        }
                            ...
                            &prepared.reciprocal_by_word[ordinal]
                        {
                            ...
                            visit(SecondMomentum20001GpuColumnEvent::Term {
                                requested_word_ordinal: ordinal,
                                term,
                            })?;
                    ...
                    if reported != current_word_components {
                        return Err(io::Error::new(
                            io::ErrorKind::InvalidData,
                            "downloaded (20001) terminal count disagrees with emitted terms",
                        ));
                    }
                    Ok(reported)
                }
    ...
    F: FnMut(SecondMomentum20001GpuColumnEvent) -> io::Result<()>,
{
    if start_word_ordinal > prepared.words.len() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "(20001) start word ordinal exceeds the PBW plan",
        ));
    }
    let mut observed_by_word = vec![false; prepared.words.len()];
    let mut emitted_terms = 0_u64;
    ...
    crate::eleven_dimensional_level16_couplings::
        ...
        |event| {
            match event {
                ...
                crate::eleven_dimensional_level16_couplings::
                    SecondMomentum20001DescendantEvent::Component(entry) => {
                    for &(momentum_pair, primitive_coefficient) in
