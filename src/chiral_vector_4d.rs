...
fn published_cv_l_matrices() -> [[[i8; 8]; 8]; 8] {
    ...
    let color_permutations: Vec<Vec<usize>> = S8_REPRESENTATION_OCTETS[2]
        .permutations
        .iter()
        .map(|permutation| {
            permutation
        ...
        .collect();
    let signs: Vec<i8> = S8_BASE_BOOLEAN_FACTORS[2]
        .iter()
...
fn field_label(field: Field) -> String {
    match field {
        ...
        Field::AuxiliaryD => "d".into(),
        Field::Psi(component) => format!("psi_{component}"),
        Field::Lambda(component) => format!("lambda_{component}"),
    }
}
...
pub fn higher_dimensional_fingerprint() -> MultipletFingerprint {
    ...
    for left in 0..charges.len() {
        for right in left..charges.len() {
            for nu in 0..DIM {
                ...
                let field = Field::Vector(nu);
                let mut residue =
                    published_closure(charges[left], charges[right], field, &clifford);
                residue.add_scaled(
                    &pure_translation(charges[left], charges[right], field, &clifford),
                    &GaussianRational::new(-1, 0),
                );
    ...
    MultipletFingerprint {
        ...
        gauge: GaugeFingerprint {
            potential_form_degree: 1,
            potential_components_before_gauge_fixing: 4,
            temporal_gauge_components_removed: 1,
            potential_components_after_temporal_gauge: 3,
            field_strength_form_degree: 2,
            gauge_parameter_form_degree: 0,
