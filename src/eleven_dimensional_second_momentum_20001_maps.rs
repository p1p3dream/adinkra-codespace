//! Exact level-12 source maps into the `(20001)` second-momentum channel.
//!
//! The four source irreps below account for nine level-12 exterior
//! embeddings. Each source tensored with one spinor contains `(20001)` once.
...
//! `(20001) -> Sym^2(V) tensor (10001)` momentum recoupling is not constructed
...
const SCHEMA_VERSION: &str = "adynkra-11d-second-momentum-20001-maps-v1";
const TARGET_DYNKIN_LABEL: &str = "20001";
const EXPECTED_TARGET_DIMENSION: u64 = 1_760;
...
pub struct SecondMomentum20001MapJob {
    pub source_dynkin_label: String,
    pub source_copy: usize,
}

impl SecondMomentum20001MapJob {
...
pub fn worklist() -> Vec<SecondMomentum20001MapJob> {
    source_fixtures()
        .into_iter()
        .map(|fixture| SecondMomentum20001MapJob {
...
pub struct SecondMomentum20001AbstractCheckpoint {
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct SecondMomentum20001EmbeddedCheckpoint {
    pub schema_version: String,
    pub job: SecondMomentum20001MapJob,
    pub source_fixture: String,
    pub source_fixture_sha256: String,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct SecondMomentum20001MutationControl {
    pub source_dynkin_label: String,
    pub source_copy: usize,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct SecondMomentum20001Report {
    pub schema_version: String,
    ...
    pub mutation_control: Option<SecondMomentum20001MutationControl>,
    pub remaining_jobs: Vec<SecondMomentum20001MapJob>,
    pub source_to_20001_component_maps_complete: bool,
    pub momentum_recoupling_20001_to_10001_complete: bool,
...
fn abstract_path(directory: &Path, source: &str) -> PathBuf {
    directory.join(format!("abstract_20001_from_{source}.json"))
}

fn embedded_path(directory: &Path, job: &SecondMomentum20001MapJob) -> PathBuf {
...
fn read_abstract(
    directory: &Path,
    source: &str,
) -> io::Result<SecondMomentum20001AbstractCheckpoint> {
    serde_json::from_reader(File::open(abstract_path(directory, source))?)
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))
}

fn abstract_is_valid(checkpoint: &SecondMomentum20001AbstractCheckpoint) -> bool {
...
pub fn construct_abstract_checkpoint(
    ...
    let certificate =
        crate::eleven_dimensional_level16_couplings::build_second_momentum_20001_abstract(
            source,
            fixture.copy,
            2,
            fixture.bytes,
    ...
    let checkpoint = SecondMomentum20001AbstractCheckpoint {
...
fn embedded_is_valid(
    checkpoint: &SecondMomentum20001EmbeddedCheckpoint,
    abstract_checkpoint: &SecondMomentum20001AbstractCheckpoint,
) -> bool {
...
pub fn construct_embedded_checkpoint(
    directory: &Path,
    job: &SecondMomentum20001MapJob,
) -> io::Result<SecondMomentum20001EmbeddedCheckpoint> {
    ...
    if path.exists() {
        let checkpoint: SecondMomentum20001EmbeddedCheckpoint =
    ...
    let (certificate, coupled_map_sha256) = crate::eleven_dimensional_level16_couplings::
        verify_second_momentum_20001_embedding_with_hash(
    ...
    let checkpoint = SecondMomentum20001EmbeddedCheckpoint {
        ...
        rank_derivation: "a nonzero B5-equivariant map onto the multiplicity-one irreducible (20001) target is surjective".to_string(),
...
pub fn construct_mutation_control(
    directory: &Path,
) -> io::Result<SecondMomentum20001MutationControl> {
    let source = "10002";
    let job = SecondMomentum20001MapJob {
        source_dynkin_label: source.to_string(),
        source_copy: 1,
    };
    ...
    let control = SecondMomentum20001MutationControl {
...
fn completed_abstracts(
    directory: &Path,
) -> BTreeMap<String, SecondMomentum20001AbstractCheckpoint> {
    source_labels()
        .into_iter()
        .filter_map(|source| {
...
fn completed_embedded(directory: &Path) -> Vec<SecondMomentum20001EmbeddedCheckpoint> {
...
pub fn summarize(directory: &Path) -> SecondMomentum20001Report {
    let abstracts = completed_abstracts(directory);
    ...
    let hashes = embedded
        ...
        .collect::<BTreeSet<_>>();
    let mutation_control: Option<SecondMomentum20001MutationControl> =
        File::open(mutation_path(directory))
            .ok()
            .and_then(|file| serde_json::from_reader(file).ok())
            .filter(|control: &SecondMomentum20001MutationControl| {
                control.source_dynkin_label == "10002"
    ...
    let maximum_csr_storage_lower_bound_bytes = abstracts
        ...
        .max()
        .unwrap_or(0);
    let source_to_20001_component_maps_complete = abstracts.len() == 4
        && embedded.len() == 9
    ...
    SecondMomentum20001Report {
        ...
        mutation_control,
        remaining_jobs,
        source_to_20001_component_maps_complete,
        momentum_recoupling_20001_to_10001_complete: false,
        physical_target_gauge_quotient_complete: false,
        full_physical_f_complete: false,
        full_f_a_g_p_zero_proved: false,
        passed: source_to_20001_component_maps_complete,
        boundary: "This certifies all nine exact level-12 source-times-spinor component maps into the (20001) intermediate channel when passed. The momentum recoupling from (20001) into Sym^2(V) tensor (10001), the physical target gauge quotient, complete F, and full F A G_p = 0 remain unproved.".to_string(),
    }
}

pub fn construct_all(directory: &Path) -> io::Result<SecondMomentum20001Report> {
    ...
    if !report.passed {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "incomplete exact (20001) second-momentum map tranche",
        ));
    }
    Ok(report)
}

/// Build or resume every `(20001)` checkpoint and atomically publish the
/// aggregate validation report.
pub fn write_artifact(
    checkpoint_directory: &Path,
    output_path: &Path,
) -> io::Result<SecondMomentum20001Report> {
...
mod tests {
    ...
    #[test]
    fn worklist_has_four_source_irreps_and_nine_copies() {
        let jobs = worklist();
        assert_eq!(jobs.len(), 9);
        assert_eq!(source_labels().len(), 4);
        assert_eq!(
    ...
    fn summary_is_fail_closed_without_exact_checkpoints() {
        ...
        assert!(!report.source_to_20001_component_maps_complete);
        assert!(!report.momentum_recoupling_20001_to_10001_complete);
        ...
        assert_eq!(report.remaining_jobs.len(), 9);
        fs::remove_dir_all(temporary).unwrap();
    }

    #[test]
    #[ignore = "constructs and checkpoints the real level-12 (10002) to (20001) map"]
    fn representative_10002_map_and_mutation_control_pass() {
        let temporary = temporary_directory("representative");
        let abstract_checkpoint = construct_abstract_checkpoint(&temporary, "10002").unwrap();
        assert!(abstract_checkpoint.passed);
        let job = SecondMomentum20001MapJob {
            source_dynkin_label: "10002".to_string(),
            source_copy: 1,
        };
    ...
    #[test]
    #[ignore = "writes all nine exact (20001) checkpoints and their aggregate report"]
    fn write_complete_20001_map_tranche() {
        let report = write_artifact(
            Path::new("results/adynkra_11d_second_momentum_20001_checkpoints"),
            Path::new("results/adynkra_11d_second_momentum_20001_maps.json"),
        )
        .unwrap();
        assert!(report.passed);
        assert_eq!(report.exact_abstract_intertwiners, 4);
        assert_eq!(report.exact_embedded_component_maps, 9);
